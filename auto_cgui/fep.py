...
from auto_cgui.FEPBrowserProcess import FEPBrowserProcess
...
def log_failure(case_info, step, elapsed_time):
    ...
    jobid = case_info['jobid']
    label = case_info['label']
    with open(LOGFILE, 'a') as f_h:
        label = case_info['label']
        f_h.write(templ.format(label, jobid, step, elapsed_time))
...
def log_success(case_info, elapsed_time):
    ...
    with open(LOGFILE, 'a') as f_h:
        label = case_info['label']
        f_h.write(templ.format(label, jobid, elapsed_time))
