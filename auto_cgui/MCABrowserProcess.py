...
def init_module(test_cases, args):
    ...
    for test_case in test_cases:
        if 'solvent_tests' not in test_case:
...
class MCABrowserProcess(CGUIBrowserProcess):
    def find_comp_row(self, comp_name, step):
        ...
        component basename"""
        selectors = {
            'molpacking': lambda: self.browser.find_by_css(".component_list table tr:not(:first-child) "
                                                           "td:nth-child(2)"),
            'solvent options': lambda: self.browser.find_by_text("Component ID"
                                                                 ).find_by_xpath('../..'
                                                                                 ).find_by_css("tr:not(:first-child) "
                                                                                               "td:nth-child(2)")
        }
        rows = selectors[step]()
        found = False
