...
import os
from auto_cgui.CGUIBrowserProcess import CGUIBrowserProcess
import yaml
import requests
...
class PBBrowserProcess(CGUIBrowserProcess):
    ...
    def download(self, save_as):
        ...
        print("downloading %s to %s" % (url, save_as))
        ...
        f_size = float(os.stat(save_as).st_size) / (1024.0 * 1024.0)
        print(f"download complete, file size is {f_size:5.2f} MB")
    ...
    def run_step0(self, p_chains, wait_text):
        browser = self.browser
        browser.choose('model', self.model)
        if (len(p_chains) > 1):
            clickcnt = 0
            while clickcnt < len(p_chains) - 1:
                browser.find_by_id('chainBtn').click()
                clickcnt += 1
        typecnt = 0
        for key in p_chains:
            p_chain = p_chains[key]
            capf = p_chain['initcap']
            capl = p_chain['endcap']
            repeat = p_chain['repeat']
            ...
            for i in range(tcnt):
                typ = 'type' + str(i + 1)
                name = p_chain[typ]['name']
                length = p_chain[typ]['leng']
                path_junk = self.getpath(self.polydic, name)
                idx = (len(path_junk) / 2) - 1
                poly_class = path_junk[2]
                fullname = path_junk[4]
                resi = self.polydic['Polymer']['sub'][poly_class]['sub'][fullname]['resi']
                if len(path_junk) > 6:
                ...
                poly_elem.click()
                poly_elem.find_by_text(poly_class).click()
